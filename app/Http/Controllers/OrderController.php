...
class OrderController extends Controller
{
    ...
    public function create_sessions(Request $request){
        ...
        try{
           ...
           if($request->ordertype == 'delivery'){
            ...
            if(auth()->check()){
                session(['billing_email' => auth()->user()->email]);
            }
            
            session(['ordertype' => $orderType,'session_string' => $randomString]);
        ...
        $response['addToCartData'] = '';
            if (session()->has('session_string')) {
                ...
                $basket = Basket::where('session',$session_string)->where('status',0)->first();
                 ...
                 session(['postalcode' => $request->postalCode,
                          'locType'    => $request->locType,
                          'serve_date' => $request->selDate,
                          'serve_time' => $request->selTime
                        ]);
                ...
                if(!$basket){
                    ...
                    $basket->city         = session('city');
                    $basket->order_type   = session('ordertype');
                    $basket->serve_date   = session('serve_date');
                    $basket->serve_time   = session('serve_time');
                ...
                $pdct_vari          = ProductVariation::with('products')->where('id',$request->product_id)->first();
    ...
    public function place_order(CheckoutFormRequest $request){
        ...
        if(session()->has('session_string') ){
            ...
            // if($request->has('same_billing')){
                ...
                $shipping_add['location_type'] = $request->s_location_type;
                
                $shipping_data = json_decode(json_encode($shipping_add)); // convert array to collection
            // }
            // else{
            //     $billing_data['location_type'] = $request->b_location_type;
            // }
            ...
            $paymeny_id = 'BUN'.$now->format("YmdHis").rand(0,10);
            ...
            // 
            if( ($pay->getResponseCode() < 50 && strlen($refNum) > 5 && strlen($txnNum) > 5 && is_numeric($resCod)) || env('APP_URL') == 'https://www.stage.bunches.ca')
            {
    ...
    public function storeAddress($data,$basket_id,$type){
        ...
        try{
            ...
            $save_add->location_type   = $address->location_type ?? '';
            $save_add->save();
            return 1;
        }
        catch(\Exception $e){
    ...
    {
            $store_id='gwca063640';
            $api_token='it5CiNdCaWj6K8IfYXIb';
            ...
            $expiry_date    = $date;
            $crypt          = '7';
            $dynamic_descriptor='Bunches Order';
            $status_check   = 'false';
    ...
    function invoiceNumberGenerate(){
        ...
        return 'BUN'.date('ymd').sprintf('%04d', $ordercount+1);
    }
    ...
    function GrandTotalCalculation($basket){
        ...
        $itemCount = $items->count() ?? 0;
            ...
            $orderTyp = session('ordertype');
            $cityName = session('city');
            $provinceName = session('province');
            ...
            if($orderTyp == 'delivery'){
                ...
                if($city){
                    $shipCharge  = LocationShippingCity::where('city_id',$city->master_id)->where('shipping_id',$shiping_method->id)->pluck('charge')->first();
                }
                elseif($province){
                    $shipCharge = LocationShipping::where('province',$province->code)->where('shipping_id',$shiping_method->id)->pluck('charge')->first();
                }
                else{
                     $shipCharge = $shiping_method->charge;
                }
               
               if($shipCharge <= 0 || $shipCharge == null ){
                   $shipCharge = $shiping_method->charge;
               }
            }
            else{
            ...
            $Calculation['ShippingCharge']  = $shipCharge;
            $Calculation['shippingTax']     = ($shipCharge * 13) / 100; 
