@extends('layouts.frontend')
    @section('styles')
        ...
        thead.ir-table th:nth-child(2), thead.ir-table th:nth-child(3), thead.ir-table th:nth-child(4) {
            border-left: none;
            border-right: none;
        }
...
@section('contents')
     <section class="bread_crumb bg-light py-3">
        <div class="container">
            <div class="row">
                <div class="col-6">
                    <p class="mb-0 f-15">Cart</p>
                </div>
                <div class="col-6 text-end">
                    <p class="mb-0 f-14 text-dark ">You are here: Bunches > Cart</p>
                </div>
            </div>
        </div>
    </section>
    ...
    @if(isset($items) && $items->count() > 0)
        ...
        @if($hasPreOrderProduct)
            ...
            @endphp
        @endif
    <section class="cart-bale page_section mb-3 pt-5">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach($items as $listing)
                                    <tr  class="cart-item2 position-relative" data-pname="{{$listing->product_name}}" data-psku="{{$listing->product_sku}}" data-pid="{{$listing->product_variation_id}}"  data-price="{{$listing->price_amount}}">
                                        <th scope="row" data-pname="{{$listing->product_name}}" data-psku="{{$listing->product_sku}}" data-pid="{{$listing->product_variation_id}}"  data-price="{{$listing->price_amount}}">
                                            <div class="d-flex align-item-center">
                                                ...
                                                <div class="product-detail-cart d-flex flex-column">
                                                    ...
                                                    <i class="text-uppercase small text-black-50 ">SKU:{{$listing->product_sku}}</i>
                                                    
                                                    @if($listing->customized_message != '') <small>Message: {{$listing->customized_message}}</small> @endif
                                                   
                                                </div>
                                            </div>
                                        </th>
                                        <td data-pname="{{$listing->product_name}}" data-psku="{{$listing->product_sku}}" data-pid="{{$listing->product_variation_id}}"  data-price="{{$listing->price_amount}}">
                                            <div class="price">
                                            ...
                                            <div class="quantity">
                                                <div class="qty-input">
                                                    <button class="qty-count qty-count--minus" data-action="minus"
                                                        type="button">-</button>
                                                    <input class="product-qty item-quantity" data-pname="{{$listing->product_name}}" data-psku="{{$listing->product_sku}}" data-pid="{{$listing->product_variation_id}}"  data-price="{{$listing->price_amount}}" type="number" name="product-qty" min="0" 
                                        ...
                                        <td data-pname="{{$listing->product_name}}" data-psku="{{$listing->product_sku}}" data-pid="{{$listing->product_variation_id}}"  data-price="{{$listing->price_amount}}">
                                            <div class="Item_total text-primary-color fw-5">
                                                {{getPrice($listing->price_amount * $listing->quantity)}}
                                            </div>
                                        </td>
                                        <td >
                                            <div role="button" class="item_remove cursor-pointer" data-preorder="{{$listing->pre_order}}" data-pname="{{$listing->product_name}}" data-psku="{{$listing->product_sku}}" data-pid="{{$listing->product_variation_id}}"  data-price="{{$listing->price_amount}}">
                        ...
                        <div class="t-price pt-2">
                            <div class="row">
                                <div class="col-12 text-end">
                                    <h3 id="total-amount" class="text-primary-color fw-bold">Total: {{getPrice($totalAmount)}}</h3>
                                </div> 
                            </div>
                        </div>
                </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-12 col-md-6 mb-3 mb-md-0">
                    <label class="text-dark mb-2">Order Remark</label>
                    <textarea class="form-control rounded-0"  name="remark" id="remark" form="goto_checkout">{{$basket->remarks}}</textarea>
                </div>
                <div class="col-12 col-md-6">
                    ...
                    @if($basket->order_type == 'pickup')
                        ...
                        @php
                         $pickup_store          = App\Models\Store::with('store_timing', 'holidaytiming', 'holidaytiming.holiday')->first();
                        ...
                        <form action="{{url('cart/get_addons')}}" id="goto_checkout" method="POST" data-callback="addon_complete" data-classes="goto_checkout" class="row" novalidate>
                            @csrf()
                             <div class="form-group col-lg-12" >
                                <label class="text-dark mb-2">Delivery date : </label>
                            </div>
                            <div class="col-lg-12  mb-2">
                                 <div class="input-group form-group ">
                                    <span class="form-control date-input cursor-pointer rounded-0"  id="date-dropdown-toggle" >{{$basket->serve_date != ''  && date('Y-m-d',strtotime($basket->serve_date)) > date('Y-m-d') ? date('F d, Y',strtotime($basket->serve_date)) : ''}}</span>
                    ...
                    @endif
                        ...
                        @if(($basket->order_type == 'pickup' && $pickup_store) || $basket->order_type != 'pickup')
                            <div class="for-checkout mt-2 col-lg-12 text-end">
                                ...
                                <input type="submit" form="cart_continue" class="btn border-0 px-3 py-2 continue_shopping_btn" value="Continue Shopping">
                                <input type="submit" form="goto_checkout" class="rounded-0 px-3 py-2 border-0 primary-dark-bg  checkout_btn btn cart-button " value="Checkout">
                            </div>
                        @endif
            ...
            <div class=" bg-white p-5">
                <div class="text-center">
                    <h1 class="ir-h1">    
                        ...
                        <a href="{{url('category')}}" class="btn  btn-sm btn-theme border">Continue shopping</a>
                </div>
            </div>
        </div>
    </div>
...
@section('scripts')
    ...
    <script>
        ...
        $('body').on('change','.item-quantity', async function() {
            ...
            var item = $(this).closest('.cart-item2');
        ...
        $('body').on('click','.item_remove', async function() {
            ...
            if ($('.cart-item2').length === 0 || preorder == 1) {
                location.reload();
            }
